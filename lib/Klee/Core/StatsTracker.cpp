...
StatsTracker::StatsTracker(Executor &_executor, std::string _objectFilename,
    ...
    if (!llvm::sys::path::is_absolute(objectFilename)) {
        SmallString<128> current(objectFilename);
        if (llvm::sys::fs::make_absolute(current)) {
#if LLVM_VERSION_CODE >= LLVM_VERSION(3, 6)
            Twine current_twine(current.str()); // requires a twine for this
            if (!llvm::sys::fs::exists(current_twine)) {
#elif LLVM_VERSION_CODE >= LLVM_VERSION(3, 5)
                bool exists = false;
                if (!llvm::sys::fs::exists(current.str(), exists)) {
#else
                bool exists = false;
                error_code ec = llvm::sys::fs::exists(current.str(), exists);
