...
//            CoreSearch("search",
//                       cl::desc("Specify the search heuristic (default=random-path interleaved with nurs:covnew)"),
//                       cl::values(clEnumValN(Searcher::DFS, "dfs", "use Depth First Search (DFS)"),
//                                  clEnumValN(Searcher::BFS, "bfs",
//                                             "use Breadth First Search (BFS), where scheduling decisions are taken at the level of (2-way) forks"),
...
//    KLEE_LLVM_CL_VAL_END));
    ...
    cl::opt<bool>
            UseBatchingSearch("use-batching-search",
                              cl::desc(
                                      "Use batching searcher (keep running selected state for N instructions/time, see --batch-instructions and --batch-time)"),
                              cl::init(false));
